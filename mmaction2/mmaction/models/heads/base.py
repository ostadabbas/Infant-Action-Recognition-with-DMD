...
class BaseHead(BaseModule, metaclass=ABCMeta):
    ...
    def predict(self, feats: Union[torch.Tensor, Tuple[torch.Tensor]],
        ...
        """
        cls_scores, extracted_features = self(feats, **kwargs)
        return self.predict_by_feat(cls_scores, data_samples, extracted_features)

    def predict_by_feat(self, cls_scores: torch.Tensor,
                        data_samples: SampleList, extracted_features) -> SampleList:
        ...
        extracted_features = extracted_features.view((batch_size // num_segs, num_segs) +
                                                     extracted_features.shape[1:])
        extracted_features = extracted_features.mean(dim=1)

        for data_sample, score, pred_label, extracted_feature in zip(data_samples, cls_scores,
                                                  pred_labels, extracted_features):
            data_sample.set_pred_score(score)
            data_sample.set_pred_label(pred_label)
            data_sample.set_pred_feats(extracted_feature)
