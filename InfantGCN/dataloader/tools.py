...
def uniform_choose(data_numpy, size, auto_pad=True):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    if T == size:
        return data_numpy
        ...
        if fair:
            ...
            choices = np.random.choice(range(T), size, replace=True)
            idxes = sorted(choices)
            ...
            idxes = np.concatenate((np.arange(0, T),
                                        padding))
            return data_numpy[:, idxes, :, :]
    else:
